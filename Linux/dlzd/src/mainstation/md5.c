...
描述:带密钥的MD5算法库
函数列表:
  1.char* MDString (char *);  //输入任意一个字符串，经过md5算法处理后，返回结果：一个定长（32个字符）字符串
  2.char* MDFile (char *);    //输入任意一个文件名，文件内容经过md5算法处理后，返回结果：一个定长（32个字符）字符串
  3.char* hmac_md5(char* text, char* key); //输入任意一个字符串text,和一个用做密钥的字符串key,
                                             经过hmac_md5算法处理，返回处理结果：一个定长字符串（32个字符）
修改历史:
  01,13-06-06,Leiyong download.
***************************************************/
