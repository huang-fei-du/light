...
修改历史：
  ...
  02,11-01-05,Leiyong,修正backTime每年1月份回退1到31天的错误.
***************************************************/
#include <stdio.h>
#include <linux/rtc.h>
#include <sys/ioctl.h>
...
DATE_TIME backTime(DATE_TIME nowTime, int month, int day, int hour,int minute,int second)
{
 	  ...
 	  if (nowTime.day < day)
   	{
   	  do
   	  {
   	    nowTime.month = (nowMonth == 1) ? 12 : (nowTime.month-1);
   	    nowTime.year = (nowMonth == 1) ? (nowTime.year-1) : nowTime.year;
    ...
    prev.month = ((nowMonth-month) == 0) ? 12 : (nowMonth - month);
    prev.year  = ((nowMonth-month) == 0) ? (nowTime.year-1) : nowTime.year;
