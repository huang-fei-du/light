...
/**************************************************
函数名称:initConcentrator
功能描述:初始电力载波集中器IO通道(控制采集单元硬件版本v0.3)
调用函数:
被调用函数:
输入参数:无
输出参数:
...
void initConcentrator(void)
{
	...
	/*1.3无线Modem复位线 - PB22*/
	if (at91_set_gpio_output(AT91_PIN_PB22, 1)!=0)
	{
	...
	/***************************5.载波模块控制引脚**************************/
	/*5.1载波模块设置(Z/SET) - PA5*/
	if (at91_set_gpio_output(AT91_PIN_PA5, 1)!=0)
	{
	...
	printk("初始化集中器IO(控制单元硬件版本v0.3,CPU为迈冲核心板BGA)\n");
}
...
void initConn9260Qfp(void)
{
	...
	/*1.3无线Modem复位线 - PA9*/
	if (at91_set_gpio_output(AT91_PIN_PA9, 1)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_PA9 Output failed!\n");
	}
	/*1.4无线Modem模块型号识别state0 - PA10*/
	...
	/***************************2.按键**************************************/
	/*2.1上键 - PA10*/
	if (at91_set_gpio_input(AT91_PIN_PA10, 1)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_PA10 Input  failed!\n");
	}
	...
	/*2.3左键 - PA22*/
	if (at91_set_gpio_input(AT91_PIN_PA22, 1)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_PA22 Input  failed!\n");
	}
	...
	/*2.5确认键 - PA8*/
	if (at91_set_gpio_input(AT91_PIN_PA8, 1)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_PA8 Input failed!\n");
	}	
	...
	/*6.2告警音 - PB24*/
	if (at91_set_gpio_output(AT91_PIN_PB24, 0)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_PB24 Output failed!\n");
	}
...
/**************************************************
函数名称:initType3Te
功能描述:初始专变III型终端IO通道(CPU为9260-912BGA)
调用函数:
被调用函数:
输入参数:无
输出参数:
返回值：
***************************************************/
void initType3Te(void)
{
	/***************************1.无线Modem*********************************/
	/*1.1无线Modem开机信号线 - PA9*/
	if (at91_set_gpio_output(AT91_PIN_PA9, 0)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_PA9 Output failed!\n");
	}
	/*1.2无线Modem开关电源线 - PB30*/
	...
	/*1.4无线Modem模块型号识别state0 - PA31*/
	if (at91_set_gpio_input(AT91_PIN_PA31, 0)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_PA31 Output failed!\n");
	}
	/*1.5无线Modem模块型号识别state1 - PC3*/
	if (at91_set_gpio_input(AT91_PIN_PC3, 0)!=0)
	{
	...
	/*1.7无线Modem模块型号识别state3 - PA30*/
	if (at91_set_gpio_input(AT91_PIN_PA30, 0)!=0)
	{
	...
	/*2.6取消键 - PA11*/
	if (at91_set_gpio_input(AT91_PIN_PA11, 1)!=0)
	{
	...
	printk("初始化GDW376-2009专变III型终端IO\n");
}
...
void initType3TeQfp(void)
{
	...
	if (at91_set_gpio_input(AT91_PIN_PA26, 0)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_PA26 Output failed!\n");
	}
	...
	if (at91_set_gpio_input(AT91_PIN_PA10, 0)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_P10 Output failed!\n");
	}
	/*1.6无线Modem模块型号识别state2 - PC1*/
	if (at91_set_gpio_input(AT91_PIN_PC1, 0)!=0)
	{
...
void initType3Tev13Qfp(void)
{
	...
	if (at91_set_gpio_output(AT91_PIN_PA10, 0)!=0)
	{
	   printk(KERN_ALERT "ioChannel:set AT91_PIN_PA10 Output failed!\n");
	}
	/*1.2无线Modem开关电源线 - PC1*/
   ...
   switch(cmd)
   {
			...
			case MACHINE_TYPE:
				...
				switch(machineType)
			  {
			  	 ...
			  	 default:   //默认为集中器(控制单元硬件版本V0.3,CPU为迈冲核心板BGA)
			  	 	 initConcentrator();
			  	 	 break;
			  }
				break;
          ...
          case 5:     //专变III型新终端(QFP)
            ...
            break;
			  	...
			  	case 6:    //集中器(控制单元硬件版本v0.4,CPU9260-QFP)
				    ...
				    {
         			case READ_KEY_VALUE:   /*读取I/O脚的值*/
         				if (!at91_get_gpio_value(AT91_PIN_PA10))
         				{
         					 tmpData = KEY_UP;
         				}
         				if (!at91_get_gpio_value(AT91_PIN_PA25))
         				...
         				if (!at91_get_gpio_value(AT91_PIN_PA8))
         				{
         					 tmpData = KEY_OK;
         				}
         				break;
